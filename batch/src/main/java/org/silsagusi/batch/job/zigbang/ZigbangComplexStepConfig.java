		...
		return (contribution, chunkContext) -> {
			while (true) {
				ScrapeStatus status = scrapeStatusRepository.findFirstByPlatformAndTargetTypeAndStatusOrderByIdAsc(
					ScrapeStatus.Platform.ZIGBANG, ScrapeStatus.TargetType.COMPLEX, ScrapeStatus.Status.PENDING);

				if (status == null) {
					break; // 더 이상 처리할 status 없음
				}

				zigbangScraper.scrapComplex(status);
				status.completed();
				log.info("Completed ZigbangComplexScraper region: {}", status.getRegion().getId());
			}
